...
const WS_URL = 'ws://localhost:3001/ws/board/'
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'user_count') {
        ...
        setNotes((prev) =>
          prev.map((n) => (n.id === msg.note.id ? { ...n, text: msg.note.text } : n))
        )
        // Clear any local edits for this note so the server's value shows
        setEditedText((prev) => {
          const next = { ...prev }
          delete next[msg.note.id]
          return next
        })
      } else if (msg.type === 'note_deleted') {
        setNotes((prev) => prev.filter((n) => n.id !== msg.id))
        setActiveNote((prev) => (prev === msg.id ? null : prev))
        setEditedText((prev) => {
          const next = { ...prev }
          delete next[msg.id]
          return next
        })
      } else if (msg.type === 'notes_cleared') {
        setNotes([])
        setActiveNote(null)
        setEditedText({})
      }
